...
const submitSignUpForm = (e, part, detailsFromSignUpWithPassword) => {
    ...
    if (detailsFromSignUpWithPassword !== undefined && details !== null) {
        ...
        details['phone_country_code'] = getCountryCallingCode(detailsFromSignUpWithPassword.phone_numberCountry);
...
const editAdmissionsForm = (e, detailsFromEditLoansForm) => {
    e.preventDefault();
    ...
    if (detailsFromEditLoansForm !== undefined && details !== null) {
        ...
        details['phone_country_code'] = getCountryCallingCode(detailsFromEditLoansForm.phone_numberCountry);
        details.phone_numberCountry = detailsFromEditLoansForm.phone_numberCountry;
    }

    fetch(`/api/v1/applications/${detailsFromEditLoansForm.application_id}/edit/admissions`,
        {
            method: "POST",
            credentials: "include",
            body: JSON.stringify(details),
        ...
        .then(response => {
                ...
                console.log("edit admissions response");
                console.log(response);
                window.location.href = window.location.origin + `/options/${detailsFromEditLoansForm.application_id}?application_type=admissions&none=${!response.has_offers}&seen_offers_before=${response.seen_offers_before}&intake_path=${response.intake_path}&program_path=${response.program_path}&home_country_path=${response.home_country_path}&school_country_path=${response.school_country_path}`;

            }
        ).catch((error) => console.log(error));
}

const editLoansForm = (e, detailsFromEditLoansForm) => {
    e.preventDefault();
    ...
    fetch(`/api/v1/applications/${detailsFromEditLoansForm.application_id}/edit/loans`,
        {
            method: "POST",
            credentials: "include",
            body: JSON.stringify(details),
        ...
        .then(response => {
            ...
            console.log(response);
            window.location.href = window.location.origin + `/options/${detailsFromEditLoansForm.application_id}?application_type=student_loan&none=${!response.has_offers}&seen_offers_before=${response.seen_offers_before}&intake_path=${response.intake_path}&program_path=${response.program_path}&home_country_path=${response.home_country_path}&school_country_path=${response.school_country_path}&cosigner_path=${response.cosigner_path}`;
            }
        ).catch((error) => console.log(error));
}
        ...
        <div className='landing-form nomad-form'>
            <Form onSubmit={e => {
                ...
                <Form.Group className="mb-3" >
                    ...
                    <Form.Label>Password*</Form.Label>
                    <br /><a href='/users/password/new' >Forgot your password?</a>
                </Form.Group>
                <Button variant="primary" size='lg' className='extra-pad' type="submit" >
                    Sign In
                </Button>
                <p style={{color:'red'}} className={validation && validation.length ? '' : 'hidden'}>{validation}</p>
                <p className='have-account mt-5'>Don't have an account? <a href='/sign-up'>Sign Up</a></p>
            </Form>
        </div>
    )
}
...
const newAdmissionsForm = (e, detailsFromNewLoansForm) => {
    ...
    if (detailsFromNewLoansForm !== undefined && detailsFromNewLoansForm !== null) {
        ...
        details['phone_country_code'] = getCountryCallingCode(detailsFromNewLoansForm.phone_numberCountry);
        details.phone_numberCountry = detailsFromNewLoansForm.phone_numberCountry;
    }

    fetch(`/api/v1/applications/admissions/new`,
        {
            method: "POST",
            credentials: "include",
            body: JSON.stringify(details),
        ...
        .then(response => {
            // TODO: Go to offers page without re-routing
            console.log("new admissions response");
            console.log(response);
            window.location.href = window.location.origin + `/options/${response.id}?application_type=admissions&none=${!response.has_offers}&seen_offers_before=${response.seen_offers_before}&intake_path=${response.intake_path}&program_path=${response.program_path}&home_country_path=${response.home_country_path}&school_country_path=${response.school_country_path}`;
            }
        ).catch((error) => console.log(error));
}
...
const newLoansForm = (e, detailsFromNewLoansForm) => {
    ...
    fetch(`/api/v1/applications/student_loan/new`,
        {
            method: "POST",
            credentials: "include",
            body: JSON.stringify(details),
        ...
        .then(response => {
            ...
            // console.log(response);
            window.location.href = window.location.origin + `/options/${response.id}?application_type=student_loan&none=${!response.has_offers}&seen_offers_before=${response.seen_offers_before}&intake_path=${response.intake_path}&program_path=${response.program_path}&home_country_path=${response.home_country_path}&school_country_path=${response.school_country_path}&cosigner_path=${response.cosigner_path}`;
            }
        ).catch((error) => console.log(error));
}
...
export const NewApplicationForm = (props) => {
    ...
    if (part === 2){
        if(type == 'admissions'){
            return(
                <AdmissionsSignupPart3 submitThisForm={submitThisForm} />
            )
        } else {
            return(
                <StudentLoanSignupPart3  submitThisForm={submitThisForm} />
            )
        }
    }
...
export const EditApplicationForm = (props) => {
    ...
    // let type = props.type;
    let {studentLoanVariant, admissionsVariant, type} = props;
    ...
    const submitThisForm = (e) => {
        ...
        if (type === 'admissions') {
            editAdmissionsForm(e, details);
        } else {
            editLoansForm(e, details);
        }
        ...
        if (type === 'admissions') {
            ...
            <>
                {studentLoanVariant && studentLoanVariant === 'test' ?
                 <StudentLoanSignupPart3B prependUniversities={true} universities={props.universities} submitThisForm={submitThisForm} application={thisApplication} />
                : 
                <StudentLoanSignupPart3A prependUniversities={true} universities={props.universities} submitThisForm={submitThisForm} application={thisApplication} />}
            </>
            )
        }
    }
...
export const SignUpForm = (props) => {
    ...
    const submitThisForm = async (e) => {
        ...
        if (details['user[email]']) {
            ...
            if (!validateEmail(details['user[email]'])) {
                ...
                await fetch(`/api/v1/users/email_exists`,
                {
                    method: "POST",
                    //credentials: "include",
    ...
    if(part === 2){
        ...
        if(interested === 'admissions'){
            ...
            return(
                <StudentLoanSignupPart3  submitThisForm={submitThisForm} universities={props.universities} />
            )
        }
    }
    return(
...
export const StudentLoanSignupPart3B = (props) => {
    ...
    const [submitting, setSubmitting] = useState(false)
    const [theseUniversities, setTheseUniversities] = useState(null)
    const [studyCountry, setStudyCountry] = useState('United States')
    const [homeCountry, setHomeCountry] = useState('India')
    const [selectedSchool, setSelectedSchool] = useState(null)
    const [schoolRadio, setSchoolRadio] = useState(null)
    let { submitThisForm, application,  universities} = props;
    let thisHomeCountry, thisCurrentCity, thisSchoolCountry, thisNeedsLoan, thisHasCosigner, thisDegreeType, thisProgram, thisIntake, thisSchool;
    
    const setUniversities = (studyCountry, universities) => {
        if(!studyCountry) return;
        if(!theseUniversities && !universities) return;
        let prependValus = [
            {label:'Still Deciding', value:'Still Deciding - ' + studyCountry},
            {label:'School Not Listed', value:'School Not Listed - ' + studyCountry},
        ];
        let cleanOldValues;
        if(universities) cleanOldValues = universities
        else cleanOldValues = [...theseUniversities].filter(x => x.label !== 'Still Deciding' && x.label !== 'School Not Listed');
    ...
    const cosignerPopover = (
        <Popover id="popover-basic">
          ...
          <Popover.Body>
            ...
            {studyCountry && studyCountry === 'United States' ? <><br /> <br /> A US Cosigner is a creditworthy US Citizen or US Permanent Resident (greencard holder).</> : ''} </p>
          </Popover.Body>
        </Popover>
      );
    ...
    let contries = ['India', 'United States', 'Canada', 'UK', 'Australia', 'New Zealand', 'Ireland', 'Europe', 'Other' ].map(x => { 
        return {value: x, label:x}
    ...
    useEffect(() => {
        if(!universities){
            ...
            //setTheseUniversities(universities)
            setUniversities(studyCountry, universities)
        }
        
        
        setHomeCountry(thisHomeCountry)
        //if(thisSchool) setSelectedSchool(thisSchool)
            ...
            <Form onSubmit={e => {
                ...
                <Form.Group className="mb-3">
                    ...
                    <Select
                        ...
                        defaultValue={studyCountry ? {label:studyCountry, value:studyCountry} : selectedOption}
                        onChange={(e,ee,eee)=>{
                            if(!e) return;
                            setStudyCountry(e.value);
                            setUniversities(e.value);
                            document.querySelectorAll('[name="application[has_cosigner]').forEach(x => {
                                x.checked = false;
                            })
                ...
                <Form.Group className="mb-3">
                    ...
                    <Form.Check
                        ...
                        name="application[has_cosigner]"
                        className={(studyCountry && studyCountry === 'United States') || (homeCountry && homeCountry === 'United States') ? '' : 'hidden'}
                        required
                    />
                </Form.Group>
                <hr />
                ...
                <Form.Group className="mb-3">
                    ...
                    <Select
                        id="application_school"
                        name="application[school]"
                        defaultValue={selectedSchool ? {label:selectedSchool, value:selectedSchool} : selectedOption}

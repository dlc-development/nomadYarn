...
const Offers = (props) => {
    ...
    useEffect(() => {
        ...
        fetch(`/api/v1/applications/${id}/offers`)
            .then(response => response.json())
            .then(response => {
    ...
    const checkWaiver = async (details) => {
        fetch(`/api/v1/applications/${id}/edit/admissions/waiver`,
            {
                method: "POST",
                credentials: "include",
                headers: {
            ...
            .then(response => response.json())
                .then(response => {
                    window.location.href = "/dashboard" + `?waiver_checked_path=${response.waiver_checked_path}&intake_path=${response.intake_path}&program_path=${response.program_path}&home_country_path=${response.home_country_path}&school_country_path=${response.school_country_path}`
                })
                .catch(error => {
                    console.log('error', ""+error);
                });
    ...
    const selectOffer = (url, i, offer, likely = false) => {
        if(applicationType === 'admissions'){
            ...
            fetch(`/api/v1/applications/${id}/offer_clicked?application_offer_id=${offer.id}&offer_id=${offer.offer_id}&likely=${likely}&application_id=${id}`
            )
            .then(response => response.json())
            .then(response => {
                if(response){
            ...
            <Row className='three-col-row'>
                <Col xs={12}>
                    ...
                    {/* <div className="new-feature">
                        <h3><FontAwesomeIcon icon={faLock} /> New Feature!</h3>
                        <p>Did you know? Our academic institution partners may require a minimum set of documents from the student to prove identification, residency, etc. These documents aid them in their decision making process. Providing documents may decrease the time it takes to get a decision. Click here to securely upload your documents NOW to Nomad's portal and we'll ensure the academic institution partners that you may be a fit for receive them.*
                            *Our lending partners are not responsible for the documents a student uploads to the site.</p>
                    </div> */}
                    {offerData.length > 0 ?
            ...
            <Row className='three-col-row'>
                {
                    likelyOffers[likelyOfferTab].map((x,i) => {
                        return(
                            <div key={i} className='offer-card'>
                                <Image style={{height: '60px'}} src={'/images/partners/' + x.offer.logo_file_name}></Image>
